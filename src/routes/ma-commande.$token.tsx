...
function OrderTrackingPage() {
  ...
  const [items, setItems] = useState<{ id: string; name: string; unit_price_cents: number; quantity: number }[]>([]);
  ...
  const load = async () => {
    ...
    setItems((itemRows ?? []) as { id: string; name: string; unit_price_cents: number; quantity: number }[]);
    setLoading(false);
  };
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="container mx-auto px-4 py-12 max-w-2xl">
        ...
        <div className="glass rounded-2xl p-6 mb-6">
          ...
          <div className="font-display text-2xl text-primary mb-4">{ORDER_STATUS_LABEL[order.status] ?? order.status}</div>
          {!finalStatus && (
            <div className="space-y-2">
              {STATUSES.map((s, i) => (
                <div key={s} className={`flex items-center gap-3 ${i <= statusIndex ? "text-foreground" : "text-muted-foreground opacity-50"}`}>
                  {i <= statusIndex ? <Check className="h-4 w-4 text-primary" /> : <Clock className="h-4 w-4" />}
        ...
        <div className="glass rounded-2xl p-6 mb-6 space-y-3">
          ...
          <div className="border-t border-border pt-3 space-y-1 text-sm">
            <div className="flex justify-between"><span>Sous-total</span><span>{formatPrice(order.subtotal_cents)}</span></div>
            {order.delivery_fee_cents > 0 && <div className="flex justify-between"><span>Livraison</span><span>{formatPrice(order.delivery_fee_cents)}</span></div>}
            <div className="flex justify-between font-display text-lg pt-2"><span>Total</span><span>{formatPrice(order.total_cents)}</span></div>
          </div>
          <div className="pt-3 border-t border-border text-sm text-muted-foreground">
            {order.order_type === "delivery" ? `Livraison • ${order.delivery_address}` : "À retirer en restaurant"}
            {" • "}{order.paid ? "Payée" : order.payment_method === "online" ? "Paiement en attente" : "Paiement sur place"}
          </div>
        </div>
