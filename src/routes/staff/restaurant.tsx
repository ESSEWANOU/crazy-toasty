...
const NEXT: Record<OrderStatus, { label: string; next: OrderStatus }[]> = {
  pending: [
    { label: "Confirmer", next: "confirmed" },
    { label: "Refuser", next: "cancelled" },
  ],
...
function RestaurantPortal() {
  ...
  useEffect(() => {
    ...
    const restaurantIds = roles
      .filter((r) => r.role === "restaurant_staff" || r.role === "manager")
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <header className="border-b border-border">
        ...
        <div className="container mx-auto px-4 pb-3 flex gap-2">
          <button
            onClick={() => setTab("live")}
            className={`rounded-full px-4 py-1.5 text-sm font-bold ${tab === "live" ? "bg-primary text-primary-foreground" : "glass"}`}
          >
            Commandes en cours
          </button>
          <button
            onClick={() => setTab("history")}
            className={`rounded-full px-4 py-1.5 text-sm font-bold ${tab === "history" ? "bg-primary text-primary-foreground" : "glass"}`}
          >
            Historique
          </button>
        </div>
      ...
      <div className="container mx-auto px-4 py-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {orders.length === 0 && (
          <p className="text-muted-foreground col-span-full text-center py-12">Aucune commande</p>
        )}
        {orders.map((o) => (
          <article key={o.id} className="glass rounded-2xl p-4 space-y-2">
            <div className="flex items-center justify-between">
              <div className="font-display text-lg">#{o.order_number}</div>
              <div className="text-xs px-2 py-1 rounded-full bg-primary/20 text-primary font-bold">
            ...
            <div className="text-sm">
              ...
              {o.delivery_address && (
                <div className="text-muted-foreground text-xs mt-1">{o.delivery_address}</div>
              )}
            </div>
            <div className="flex items-center justify-between pt-2 border-t border-border">
              <span className="font-display">{formatPrice(o.total_cents)}</span>
              <span className="text-xs text-muted-foreground">
                {o.paid ? "✓ Payée" : o.payment_method === "online" ? "CB en attente" : "Sur place"}
              </span>
            </div>
              ...
              <div className="flex gap-2 flex-wrap pt-2">
                {NEXT[o.status].map((a) => (
                  <button
                    ...
                    className={`rounded-full px-3 py-1.5 text-xs font-bold ${a.next === "cancelled" ? "bg-destructive text-destructive-foreground" : "bg-primary text-primary-foreground"}`}
                  >
                    {a.label}
                  </button>
                ))}
